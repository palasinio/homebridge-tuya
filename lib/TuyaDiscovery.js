    ...
    _onDgramMessage(port, msg, info) {
      ...
      //  this.log.info(`Discovery - UDP from ${info.address}:${port} 0x${msg.readUInt32BE(0).toString(16).padStart(8, '0')}...0x${msg.readUInt32BE(len - 4).toString(16).padStart(8, '0')}`);
        ...
        try {
            ...
            if (!result) throw new Error('No valid JSON discovery payload found');

            if (result && result.gwId && result.ip) this._onDiscover(result);
            else this.log.error(`Discovery - UDP from ${info.address}:${port} decrypted`, parsedPayload || cleanMsg.toString('hex'));
        } catch (ex) {
            ...
            if (fallbackResult) {
                this.log.info(`Discovery - Recovered malformed discovery payload on port ${port} for ${fallbackResult.gwId}.`);
                this._onDiscover(fallbackResult);
                return;
            }
